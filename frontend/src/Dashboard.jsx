...
import { getBalance, getTransactions, transfer, setupMfa, confirmMfa, setToken, clearToken } from './api';
...
export default function Dashboard({ email, onLogout }) {
  ...
  const handleTransfer = async () => {
    setTxMsg(''); setTxError('');
    if (!toAccount.trim()) { setTxError('Enter recipient account number'); return; }
    const amountCents = Math.round(parseFloat(amount) * 100);
    if (!amountCents || amountCents <= 0) { setTxError('Enter a valid amount'); return; }
  ...
  const handleMfaSetup = async () => {
    ...
    try {
      ...
      setMfaError(err.response?.data?.detail || 'Failed to generate MFA QR code');
    } finally {
      setMfaLoading(false);
    }
  };
  ...
  const handleMfaConfirm = async () => {
    ...
    try {
      ...
      const res = await import('./api').then(m => m.default.post('/mfa/confirm-auth', { code: mfaCode }));
      setMfaMsg('🔐 MFA enabled successfully!');
      setQrCode(null); setMfaCode(''); setMfaPanel(false);
    } catch (err) {
    ...
    <div className="dashboard-layout">
      ...
      <nav className="navbar">
        ...
        <div className="navbar-actions">
          ...
          <button className="btn btn-ghost btn-sm" onClick={() => setMfaPanel(!mfaPanel)}>
            🔐 MFA Setup
          </button>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="balance-card">
              ...
              <p className="balance-amount">£{(balance / 100).toLocaleString('en-GB', { minimumFractionDigits: 2 })}</p>
            ...
            {mfaPanel && (
              <div className="card card-gradient">
                <p className="section-title"><span>🔐</span> Set Up Two-Factor Authentication</p>
                ...
                {!qrCode ? (
                  <button className="btn btn-primary" onClick={handleMfaSetup} disabled={mfaLoading}>
                    {mfaLoading ? <span className="spinner" /> : 'Generate QR Code'}
                  </button>
                ) : (
                  <>
                    ...
                    <div className="form-group">
                      <label className="form-label">Enter code to confirm</label>
                      <input
                    ...
                    >
                      {mfaLoading ? <span className="spinner" /> : 'Enable MFA ✓'}
                    </button>
                  </>
                )}
              </div>
            ...
            {/* Transfer Form */}
            <div className="card">
              <p className="section-title"><span>💸</span> Send Money</p>
            ...
            <div className="card">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                <p className="section-title" style={{ marginBottom: 0 }}><span>📋</span> Recent Transactions</p>
                  ...
                  {transactions.map(tx => (
                    <div className="tx-item" key={tx.id}>
                      ...
                      <span className={`tx-amount ${tx.type}`}>
                        {tx.type === 'debit' ? '-' : '+'}{tx.amount_display}
                      </span>
                    </div>
                  ))}
                </div>
