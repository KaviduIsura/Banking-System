...
2. The backend verifies the password hash. If valid, the backend _does not_ immediately issue an access token. Instead, it prompts the frontend to ask for a 6-digit MFA code.
...
3. If the transfer amount exceeds the automated threshold (e.g., LKR1,000), the system intercepts the transfer, placing it in a `pending_review` state rather than settling the funds immediately.
